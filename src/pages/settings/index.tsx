...
export default function SettingsPage() {
  ...
  return <Page title='设置'>
    <View className='about-hero'><Image src={asset('mascot.png')} mode='aspectFit' /><Text className='page-title'>吃饭这件事，可别忘了。</Text><Text className='muted'>今天吃啥 · 2.0</Text></View>
    ...
    <View className='settings-list'><Button className='setting-link' onClick={async () => { if (await confirm('重置全部食物库？', '所有分类的自定义食物及转盘会恢复默认，偏好设置保留。此操作不能撤销。')) { update(s => ({ ...s, scenes: initialState().scenes })); toast('食物库已恢复默认') } }}><Icon name='refresh' /><Text>重置全部食物库</Text><Icon name='chevron-right' size={18} /></Button><Button className='setting-link' onClick={() => setReward('source')}><Icon name='info-circle' /><View className='setting-copy'><Text>获取项目源码</Text><Text className='fine-print'>完整观看视频广告后可复制地址</Text></View><Icon name='chevron-right' size={18} /></Button>
      <Button className='setting-link' onClick={() => setReward('feedback')}><Icon name='users' /><View className='setting-copy'><Text>反馈建议</Text><Text className='fine-print'>完整观看视频广告后可打开反馈</Text></View><Icon name='chevron-right' size={18} /></Button>
    </View>
    </View>
